...
function wpgmza_register_ugm_version() {
    ...
    if ($current_version['version'] != $wpgmza_ugm_version) {
        ...
        if ($founded < 1) { echo $wpdb->query("ALTER TABLE $wpgmza_tblname ADD approved TINYINT(1) DEFAULT '1'"); }
    }
...
function wpgmaps_ugm_user_form($mapid) {
    ...
    if ($ugm_display) {
        ...
        if ($wpgmza_settings_ugm_autoapprove == "yes") { $wpgmza_autoapprove_checked = "1"; } else { $wpgmza_autoapprove_checked = "0"; }
        $r_msg = "<h2 style=\"clear:both;\">".__("Add your own marker","wp-google-maps")."</h2>";
        ...
        $r_msg .= "             <td><input id='wpgmza_ugm_add_title' name='wpgmza_ugm_add_title' type='text' size='35' maxlength='200' value='' placeholder='".__("Marker Title","wp-google-maps")."' /></td>";
        $r_msg .= "         </tr>";
        $r_msg .= "         <tr>";
        $r_msg .= "             <td valign=\"top\">".__("Marker Address or GPS Location","wp-google-maps")."</td>";
        $r_msg .= "             <td><input id='wpgmza_ugm_add_address' name='wpgmza_ugm_add_address' type='text' size='35' maxlength='200' value='' placeholder='".__("Marker Address","wp-google-maps")."' /><p class='desc'>".__("Or click on the map and drag to add a marker","wp-google-maps")."</p></td>";
        $r_msg .= "         </tr>";
        $r_msg .= "         <tr>";
        ...
        if ($result->ugm_category_enabled == 2) { } else { 
            ...
            if ($filterbycat_type == "1") {
                $r_msg .= "             <td><select name=\"wpgmza_category\" id=\"wpgmza_category\">".wpgmza_pro_return_category_select_list($mapid)."</select></td>";
            } else {
                $r_msg .= "             <td>".wpgmza_pro_return_category_checkbox_list($mapid,false,true)."</td>";
            }
            $r_msg .= "         </tr>";
        }
...
function wpgmaps_user_head_ugm() {
    if (isset($_POST['wpgmza_ugm_spm']) && $_POST['wpgmza_ugm_spm'] != '') {
        ...
        if (isset($wpgmza_settings['wpgmza_settings_ugm_autoapprove'])) { $wpgmza_settings_ugm_autoapprove = $wpgmza_settings['wpgmza_settings_ugm_autoapprove']; } else { $wpgmza_settings_ugm_autoapprove = "yes"; }
          ...
          $approved_setting = 0;
          if ($wpgmza_settings_ugm_autoapprove == "yes") {
              $approved_setting = 1;
          }  else {
              $approved_setting = 0;
          }
