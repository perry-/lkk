...
function un_get_form_fields($options){
	...
	if (un_get_option(UN_FEEDBACK_FORM_SHOW_EMAIL)){
		$fields['email'] = array(
			...
			'validators' => !$options['external'] && is_user_logged_in() ? array() : array('email')
			);
	}
...
function un_script($options = array()){
	...
	<script type="text/javascript">
		var usernoise = <?php echo json_encode(array(
			'config' => array(
				'loggedIn' => /*is_user_logged_in(),*/false,
				'button' => array(
          ...
          "html2canvasproxy" => usernoise_url() . "proxy.php"
				...
				'comments' => array(
					'enabled' => un_get_option(UN_COMMENTS_ENABLE, true)
				),
				'screenshot' => array(
