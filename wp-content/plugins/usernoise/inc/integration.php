...
class UN_Integration {
	...
	public function _init(){
		...
		wp_enqueue_style('usernoise-discussions', usernoise_url('/js/discussion/dist/discussion.css'), null, UN_VERSION);
	...
	public function refresh_cookie(){
		if (!isset($_COOKIES['un_unique'])){
			setcookie('un_unique', $id = wp_generate_password(), time() + 60 * 60 * 24 * 365);
		} else if (isset($_COOKIES['un_unique'])){
			setcookie('un_unique', $id = $_COOKIES['un_unique'], time() + 60 * 60 * 24 * 365);
		}
		return $id;
	}	
}
